...
public class Exam21 {
	...
	public boolean insertUser() {
		try {
			...
			String sql = "insert into user(id, pwd, name, age)"	//수정된부분
					+"value(?,?,?,?)";							//
				
			PreparedStatement prestmt = con.prepareStatement(sql);		//아래 추가됨
			prestmt.setString(1, valueHm.get("id"));
			prestmt.setString(1, valueHm.get("pwd"));
			prestmt.setString(1, valueHm.get("name"));
			prestmt.setString(1, valueHm.get("age"));					//여까지 추가됨
